...
function formatComments(comments, warnings, attachments) {
	const parts = [];
	for (const c of comments) {
		...
		const conv = Converter.convert(c.body);
		for (const x of conv.warnings) warnings.add(x);
		for (const a of conv.attachments) attachments.push(a);
		parts.push(`${heading}\n\n${conv.result}`);
	}
	return parts.length ? `\n\n## Comments\n\n${parts.join('\n\n')}` : '';
}
...
(async () => {
	try {
		...
		const { result, warnings, attachments } = Converter.convert(adf);

		const heading = summary ? `# ${summary}\n\n` : '';
		const commentsSection = formatComments(comments, warnings, attachments);
		const attachmentsSection = formatAttachments(attachments);
		process.stdout.write(heading + result + attachmentsSection + commentsSection + '\n');
